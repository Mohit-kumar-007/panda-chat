...
const initGoogle = () => {
  ...
  if (!key || key.includes('<') || key.trim() === '') return;
  try {
    googleTranslate = new Translate({ key });
    console.log('✅ Google Translate API initialised');
  } catch (e) {
    console.warn('⚠️  Failed to init Google Translate:', e.message);
  }
};

initGoogle();

// ─── Pure-Korean dictionary (last-resort local fallback) ─────────────────────
 ...
 * Returns null if not found (never returns mixed or English text).
 */
const dictLookup = (text) => {
  const key = text.toLowerCase().trim().replace(/[!?.,]+$/, '');
  if (DICT[key]) return DICT[key];

  // Try word-by-word — only if ALL words are in dict (pure Korean result)
...
// Limit: ~500 words/day (unauthenticated). Good enough for a chat app.
...
const myMemoryTranslate = async (text) => {
  ...
  try {
    ...
    // Check the result is actually Korean (contains Hangul characters)
    ...
    clearTimeout(timeout);
    throw err; // bubble up so caller can try next fallback
  }
};

// ─── Lingva Translate — another free proxy, no key ───────────────────────────
 ...
 * Priority:
 *   1. Google Translate API (if key provided)
 *   2. MyMemory free API
 *   3. Lingva free proxy
 *   4. Local dictionary
 *   5. Pure Korean fallback phrase ("새 메시지")
 *
 * The result is ALWAYS pure Korean (Hangul), never English.
 *
 * @param {string} text - Input text (any language)
 * @returns {Promise<string>} Pure Korean translation
 */
const translateToKorean = async (text) => {
  ...
  // 0. Local dictionary FIRST for common words/phrases (100% accurate, instant)
  const dictResult = dictLookup(input);
  if (dictResult) return dictResult;

  // 1. Google Translate (best quality, if key provided)
  if (googleTranslate) {
    try {
      const [result] = await googleTranslate.translate(input, { to: 'ko' });
      const hasHangul = /[\uAC00-\uD7A3]/.test(result);
      if (hasHangul) return result;
    } catch (err) {
      console.warn('Google Translate error:', err.message);
    }
  }

  // 2. MyMemory (free, no key, very reliable)
  try {
  ...
  // 4. Ultimate fallback — pure Korean so TTS always speaks Korean
  return '메시지가 왔어요';
};
